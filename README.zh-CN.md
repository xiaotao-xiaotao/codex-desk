...
Codex Desk 是一个隐私优先的 Codex CLI 桌面控制台：在桌面悬浮显示额度状态，提供账户与额度概览、额度阈值提醒、本地会话浏览、活动与 Token 趋势、详情洞察和跨设备迁移。数据通过本机 `codex app-server --stdio` 获取，不上传到第三方服务，也不读取或保存 `auth.json`。
...
![Codex Desk 文件变更详情](docs/screenshots/file-changes-zh.png)
...
- **Token 洞察**：本机 Codex app-server 提供使用数据时，可查看近 3、7 或 30 天的每日 Token 使用趋势和合计。
- **本地会话**：读取本机非归档的持久化会话，每页展示 10 个；支持按标题或会话 ID 模糊搜索，并同时显示创建时间与最后更新时间。
- **会话详情与洞察**：查看用户消息和 Codex 回复、复制单条内容；剪贴板支持时可连同图片以富文本复制，双击图片可打开独立预览，并汇总消息数、工具调用数、文件变更数和异常数；可展开查看最近 30 条结构化操作记录。
- **文件变更对比**：可从“近期操作”打开已记录的文件变更，支持左右对比和上下两种 diff 视图；展示的是会话历史中保存的差异，不读取当前工作区文件。
...
- **本机诊断与数据边界**：诊断只检查本机 CLI、`app-server` 和额度读取状态；账户、额度、Token 和会话数据均通过本机 `codex app-server --stdio` 获取，不读取或解析本地 JSONL，也不会读取或保存 `auth.json`。
...
- 展开后可拖动标题区域改变悬浮位置。
- 每 60 秒自动刷新，底部状态栏会显示下次刷新倒计时；点击刷新按钮可立即更新。
